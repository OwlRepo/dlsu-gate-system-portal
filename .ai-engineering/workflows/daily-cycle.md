...
Do not confuse this with the backend's real cron jobs: `database-sync` runs at 09:00 and 21:00 Asia/Manila (see `docs/ai/architecture-manifest.md`, Jobs/Automations). That schedule is production infrastructure syncing the student/employee roster and BioStar device state — unrelated to this AI-workflow cycle. Do not invent a parallel AI-schedule near those times; it is a real incident-time confusion risk in an on-call context.
