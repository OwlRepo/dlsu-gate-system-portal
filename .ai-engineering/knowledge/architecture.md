...
- **Design system**: no `DESIGN.md`, Storybook config, or standalone style-guide file exists. UI conventions live in `apps/portal-web/src/components/ui/` (shadcn/Radix primitives) and Tailwind config. Follow those existing patterns; flag if a UI task seems to need a real design-system doc and none exists yet.
...
- `apps/backend/src/app.controller.ts` — dead, no routes. `test/app.e2e-spec.ts` expects `GET /` and is likely stale/failing.
- `apps/portal-web/src/middleware.ts` — matcher list includes literal `"dashboard"` (missing leading slash), never matches; role-based redirect logic exists but is commented out.

See `testing-strategy.md` for verification commands (not duplicated here — one canonical home for commands).
