...
| Directory | Purpose |
| --- | --- |
| `docs/ai/` | AI workflow navigation docs (this file's directory) — maps only, not proof. |

## Update Status

Last refreshed: 2026-07-08 (single-agent Claude workflow migration pass).

Stale entries: none known at time of writing — verify against source before relying on this map for a Deep task.

Missing entries: shared `packages/` workspace existence unconfirmed; deployment target docs unconfirmed; CI workflow files unconfirmed.
